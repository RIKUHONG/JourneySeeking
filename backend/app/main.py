...
app = FastAPI(title="MiliTravel API", version="0.1.0", description="旅行规划后端 API")
...
async def request_validation_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
    body = ErrorResponse(code="INVALID_TRIP_REQUEST", message="请求参数不符合行程约束", request_id=_request_id(request))
